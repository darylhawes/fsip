...
var DATAFOLDER = FOLDER_PREFIX + 'data/';
var IMAGES = DATAFOLDER + 'images/';
var LIB = FOLDER_PREFIX + 'lib/';
var INCLUDES = LIB + 'includes/';
var IMGFOLDER = LIB + 'images/';
var JS = INCLUDES + 'js/';
...
function updateTags(that) {
	var container = $(that).closest('.image_tag_container');		
	var tags_html = tags.map(function(item) { return '<img src="' + BASE + IMGFOLDER + 'icons/tag.png" alt="" /> <a class="tag">' + item + '</a>'; });
	...
	$("button").each(function() {
		if($(this).hasClass('buttonized')) {
			...
			if (disabled != 'disabled') {
				$(this).prepend('<img src="' + BASE + IMGFOLDER + 'actions/' + button + '.png" alt="" height="12" width="' + width + '" /> ');
			} else {
				$(this).prepend('<img src="' + BASE + IMGFOLDER + 'actions/' + button + '.png" alt="" height="12" width="' + width + '" style="opacity:.5;" /> ');
			}
...
$(document).ready(function() {
	...
	if (jQuery().tipTip) {
		...
		$('input.tip').tipTip({defaultPosition: 'left', delay: 0});
	}
	...
	*/
	...
	$(".date").datepicker({
		showOn: 'button',
		buttonImage: BASE + IMGFOLDER + 'icons/calendar.png',
