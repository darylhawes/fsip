	...
	foreach($image_ids as $image_id) {
		...
		if (@$_POST['image-' . $image_id . '-delete'] == 'delete') {
			...
			$fields = array('image_title' => $image_title,
				...
				'image_geo' => makeUnicode(@$_POST['image-' . $image_id . '-geo']),
...
addNote('Your shoebox has been processed.', 'success');
...
if ($i_count == 0) {
	addNote('There are no files in your shoebox.', 'error');
...
<form action="" method="post">
	<div id="privacy_html" class="none">
		<?php echo showPrivacy('image--privacy'); ?>
	</div>
	
	<div id="rights_html" class="none">
		<?php echo showRights('right--id'); ?>
	</div>
	...
	<p>
		...
		<input id="shoebox_add" type="submit" value="Save changes" /> or <a href="<?php echo back(); ?>">cancel</a>
	</p>
</form>
