...
$user = new User;
$user->hasPermission('rights', true);
...
if (!empty($_POST['right_id'])) {
	$right_id = findID($_POST['right_id']);
	...
	if (!empty($_POST['right_delete']) and ($_POST['right_delete'] == 'delete')) {
		if ($right->delete()) {
			addNote('The rights set has been deleted.', 'success');
		}
	} elseif (!empty($_POST['right_recover']) and ($_POST['right_recover'] == 'recover')) {
		if ($right->recover()) {
			addNote('The right set has been recovered.', 'success');
		}
		...
		if (!empty($_POST['right_markup'])) {
			...
			$right_markup_ext = returnConf('web_markup_ext');
			$right_description = $orbit->hook('markup_' . $right_markup_ext, $right_description_raw, $right_description_raw);
			$right_title = $orbit->hook('markup_title_' . $right_markup_ext, $right_title, $right_title);
		} else {
			$right_markup_ext = '';
			$right_description = fsip_nl2br($right_description_raw);
		}
		
		$fields = array('right_title' => makeUnicode($right_title),
			'right_description_raw' => makeUnicode($right_description_raw),
			'right_description' => makeUnicode($right_description));
...
<?php
		...
		foreach($rights->rights as $right) {
			echo '<tr class="ro">';
				echo '<td><strong class="large"><a href="' . BASE . ADMINFOLDER . 'rights' . URL_ID . $right['right_id'] . URL_RW . '" class="tip" title="' . htmlentities(fitStringByWord(strip_tags($right['right_description']), 150)) . '">' . $right['right_title'] . '</a></strong></td>';
				...
				echo '<td>' . formatTime($right['right_created']) . '</td>';
				echo '<td>' . ucfirst(formatRelTime($right['right_modified'])) . '</td>';
			echo '</tr>';
		}
...
?>
	...
	<form id="rights" action="<?php echo BASE . ADMINFOLDER . 'rights' . URL_CAP; ?>" method="post">
			...
			<div class="span-8 last">
				<table>
					<tr>
						...
						<td>
							<label for="right_merge">Transfer images to <?php echo showRights('right_merge_id'); ?> rights set.</label><br />
		...
		<p>
			<input type="hidden" name="right_id" value="<?php echo $right['right_id']; ?>" /><input type="submit" value="Save changes" /> or <a href="<?php echo back(); ?>">cancel</a>
		</p>
	</form>
