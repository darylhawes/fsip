...
class Flickr extends Orbit {
	...
	public function orbit_config() {
		...
		if($this->flickr_active){
			$this->flickr_format_image = makeHTMLSafe($this->flickr_format_image);
			?>
			<table>
				<tr>
					<td class="right"><label>Username:</label></td>
					<td><a href="http://flickr.com/photos/<?php echo $this->flickr_nsid; ?>/"><?php echo $this->flickr_username; ?></a> &#0160; <a href="<?php echo locationFull(array('unlink' => 'flickr')); ?>"><button>Unlink from Flickr</button></a></td>
				</tr>
				<tr>
			...
			<table>
				<tr>
					<td class="right"><label>Username:</label></td>
					<td>
						<a href="<?php echo locationFull(array('link' => 'flickr')); ?>"><button>Link to Flickr</button></a><br /><br />
		...
		if (!empty($_GET['from'])) {
			switch($_GET['from']) {
				case 'flickr':
					...
					// setPref('flickr_screen_name', $user->screen_name);
					
					$this->setPref('flickr_token', $result['token']);
					
					$this->savePref();
					
					addNote('You successfully linked your Flickr account.', 'success');
					$location = location();
		...
		if (!empty($_GET['link'])) {
			switch($_GET['link']) {
				case 'flickr':
					...
					$params = array('to' => $to,
						'from' => locationFull(array('from' => 'flickr')));
		...
		if (!empty($_GET['unlink'])) {
			switch($_GET['unlink']) {
				case 'flickr':
					...
					addNote('You successfully unlinked your Flickr account.', 'success');
					headerLocationRedirect(location());
					exit();
